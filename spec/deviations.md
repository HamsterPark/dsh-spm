...
## D-LANG-? · `pyFixed` 的**符号与半分点都不能从算出来的那个数里读**
...
当时各挡了一层绕开 kernel。前者已撤（十处调用直接走 `pyFixed`，行为一字未变）；
**后者没动** —— 这一轮另有一条支线正在 `vision/` 与 `numerics/` 之间搬家，
分派单点名不许碰那两个包。它现在是一句恒假的判断，**下一个动 `vision/scan-prep.ts`
的人请顺手撤掉**（`fx()` 里 `if (Object.is(x, -0))` 那两行连同上面那段注释）。
