...
## D-PAPER-? · `SegmentRegion_UNet` / `DetectAtoms_FCN` 的 ML 支不移，而 `method` **说的是真的跑了哪一条**
...
## D-PAPER-? · `diff_scans` 的 `ndim != 2` 闸：本仓的等价条件是 `rows === 1`，`(1,1)` 那一格对不上
...
## D-PAPER-? · `_resolve_output_path` 的「同一个文件」判据：本仓**不碰文件系统**
...
## D-PAPER-? · `fftconvolve(·, psf, 'same')` 用 `correlate2d` 拼出来 —— **偶数核要补一行零**
...
## D-MANIP-? · `MoveAtomTo` 「仪器没还原」那句话里印的是 `None`，**照移，理由在这里**
...
## D-GRID-? · `GridSTS` 的三条分支在声明范围内**够不着**
...
## D-SAVE-? · `SaveScan` 的 `findLatestSxm` 接上了文件系统
