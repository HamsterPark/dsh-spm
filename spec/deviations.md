...
## D-SCANPATH-1 · `ScanAt` 从不返回路径 —— **两段死代码，新旧两仓都是**
...
## D-COARSE-1 · `phase_shift` 回 `None` 时旧仓抛 `TypeError`，本仓给一句拒绝
...
## D-POINT-1 · `no numeric value in Nanonis reply:` 后面印的是 body，不是信封
...
## D-GRID-1 · 超时判据在**通用轨迹金样里是死代码**，它的输入在 `batch7b2.json`
...
## D-OPTICS-1 · `AcquireSignalPoint` 不是 D 档 —— 它一次都不碰 registry
...
## D-PHASESHIFT-1 · `phase_shift` 的归一化分母是 `1e-30`，**不是** `100·eps`
