...
/**
 * 2026-09-19 收尾：批 6b 报上来 `pyFixed(-0, n)` 丢负号，去钉的时候把整个函数
 * 对着 CPython 3.13 跑了一遍 —— **10 843 个 double × 7 档小数位 = 75 901 格**
 * （含全部 `n/2^k` 半分点、一万个伪随机 double），当时有 **70 格**不一致。
 *
 * 下面三组是那 70 格的**三个类**各自的代表，逐个与
 * 参考系统所用 CPython 3.13 的
 * `"%.*f" % (n, x)` 对过。改完之后 75 901 格零分岔。
 *
 * 为什么用 `it.each` 列表而不是一句「跑一万个随机数对一对」：同 `pyRound` 那一组的
 * 理由 —— **分岔在均匀采样里密度接近零**（70/75901 ≈ 0.09%，而其中 ② 那一类
 * 全部落在「乘 10^(d+1) 之后恰好撞上整数」的那几个 double 上）。判据要由
 * **构造**保证，不由抽样保证。
 */
describe('pyFixed —— 与 CPython 逐个对过的三类分岔（2026-09-19）', () => {
  /** ① 符号取自**输入**：`toFixed` 判 `x < 0`，而 `-0 < 0` 是假。 */
  it.each([
    [-0, 0, '-0'],
    [-0, 1, '-0.0'],
    [-0, 2, '-0.00'],
    [-0, 3, '-0.000'],
    [-0, 6, '-0.000000'],
    // **常见情形**：不是 `-0` 本身，是四舍五入之后变成零、符号还在的数。
    [-1e-9, 3, '-0.000'],
    [-1e-12, 3, '-0.000'],
    [-1e-8, 3, '-0.000'],
    [-0.0004, 3, '-0.000'],
    ...
    [1e-9, 3, '0.000'],
    [0.0004, 3, '0.000'],
  ])('①「%f」@%d 位 → %s（负号来自输入）', (v, d, want) => {
    expect(pyFixed(v, d)).toBe(want)
  })

  /**
   * ② 半分点的判据是**精确**的。这几个数「乘 10^(d+1) 之后恰好是整数、末位是 5」
   * —— 上一版据此判成半分点并取偶，而它们的**精确值**根本不在半分点上。
   * 这正是 `pyRound` 抬头第一条警告的那种写法：**那个乘法自己要舍入**。
   */
  it.each([
    [2.675, 2, '2.67'], // 精确值 2.674999999999999822…，而 2.675*1000 恰好 = 2675
    [-2.675, 2, '-2.67'],
    [2.665, 2, '2.67'], // 精确值 2.66500000000000003552…
    [-2.665, 2, '-2.67'],
    [1.115, 2, '1.11'],
    [0.155, 2, '0.15'],
    [0.165, 2, '0.17'],
    ...
    [0.0115, 3, '0.011'],
    [0.0065, 3, '0.006'],
  ])('②「%f」@%d 位 → %s（看着像半分点，精确值不是）', (v, d, want) => {
    expect(pyFixed(v, d)).toBe(want)
  })

  /** ③ 两者叠在一起：半分点分支算出零，符号在那一步一起没了。 */
  it.each([
    [-0.5, 0, '-0'], // 真半分点，取偶得 0 —— 而 CPython 印 `-0`
    [0.5, 0, '0'],
    [-0.05, 1, '-0.1'], // 假半分点 + 负号
    [0.05, 1, '0.1'],
    [-0.005, 2, '-0.01'],
    ...
    [-0.00005, 4, '-0.0001'],
    [-0.000005, 5, '-0.00001'],
  ])('③「%f」@%d 位 → %s（负号 + 半分点叠在一起）', (v, d, want) => {
    expect(pyFixed(v, d)).toBe(want)
  })

  /**
   * ②' 真半分点仍然取偶 —— 上面那一类**不能**是把半分点分支整个删掉换来的。
   * 这一组与分支存在与否互为反证：删掉分支，这里全红；保留旧判据，上面全红。
   */
  it.each([
    [0.0625, 3, '0.062'],
    [0.1875, 3, '0.188'],
    ...
    [2.5, 0, '2'],
    [-2.5, 0, '-2'],
    [1024.0625, 3, '1024.062'],
  ])("②' 真半分点「%f」@%d 位 → %s（取偶）", (v, d, want) => {
    expect(pyFixed(v, d)).toBe(want)
  })

  it('这三类在均匀采样里几乎碰不到 —— 所以它们是**构造**出来的，不是抽出来的', () => {
    // 70 / 75901 ≈ 0.09%，而 ② 那一类要求「乘 10^(d+1) 之后恰好落在整数上」。
    // 一条每跑一次换一批输入的测试，红了说不清是回归还是运气（同 pyRound 那一组）。
    let seed = 20260919 >>> 0
    let hits = 0
    for (let i = 0; i < 200_000; i += 1) {
      seed = (Math.imul(seed, 1664525) + 1013904223) >>> 0
      const x = (seed / 4294967296) * 24 - 12
      // 旧判据：`v · 10^(d+1)` 是整数且末位 ±5
      const scaled = x * 10 ** 3
      if (Number.isInteger(scaled) && Math.abs(scaled % 10) === 5) hits += 1
    }
    expect(hits).toBe(0)
  })
})
