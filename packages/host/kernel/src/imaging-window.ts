 ...
 * 而不是依赖上一个 skill**。」
 */
// **这个文件不再自带工作点常量**（2026-09-17 合并时收的）。
//
// 批 4b 与批 4d 在同一轮里各自从 2026-08-26 那次事故移了一份，值一样、键名一个
// snake 一个 camel —— 于是 kernel 的 `export *` 当场撞车（TS2308）。留下的是
// `scan-resolver.ts` 那一份：它**有消费方**（`resolveScan` 在用），而这里这一份
// 连同 `atomicWorkingPoint()` 全仓零消费方。
//
// 十份 `cell()` 那一课在并行下有个新的后半句：**两条支线看不见彼此，
// 所以「让它只有一份」这条纪律在并行时失效，必须由合并方补。**
// 要工作点的，从 `scan-resolver.js` 取 `ATOMIC_WORKING_POINT`。
