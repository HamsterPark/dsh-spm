 ...
 * 跑在 UTC+8。vitest.config.ts 在测试 worker 启动前固定 Asia/Shanghai，
 * 让各平台重放同一导出环境；生产行为与金样文本均保持不变。
 * 这条守卫继续检查时区前提，配置失效时给出明确原因。
 */
  ...
  it('这台机器的时区渲染与金样里那一格一致（不一致就是时区，不是回归）', () => {
    ...
    expect(
      ...
      'spec/golden/skill_traces.json 的 ReadCalibrations 三格 age_note 里编码的是 ' +
        ...
        here +
        '，请检查 vitest.config.ts 是否在 worker 启动前将 TZ 固定为 Asia/Shanghai。',
