 ...
 * `what` 是读取器报错时印的那个名字。
 *
 * ⚠️ **缺省仍然是 `readSxm` 自己的 `<sxm>`**，不是路径。
 *
 * 初稿这里写的理由是「批 4a/4c/6b/6c 的金样里那几句报文印的就是 `<sxm>`，改缺省会让
 * 它们整排变红」—— **2026-09-20 核出来：不成立。** `<sxm>` 在整个 `spec/golden/` 里
 * 出现 **0 次**；含那句 `header end marker` 报文的金样只有三份（`batch7a3` /
 * `nanonis_files` / `tilt`），都不属于那四批。7a-3 的交接自己也写着「零影响」。
 *
 * **真正的理由**：`loadSxm` 全仓 12 个调用点、**10 个吃缺省**，而没有一格金样看着它们。
 * 改缺省 = 同时改掉 10 处**面向模型的报文**，而**不会有任何一格变红**。
 * ⇒ 不改缺省，不是因为「会打破测试」，恰恰是因为**不会**：
 * **在一个没有判据看着的地方改模型读的那句话，错了也没人会知道。**
 *
 * （这两个理由的方向是相反的。编出来的那个听起来更硬 —— 「会打破测试」像一条事实；
 * 真的那个是「测不到」，听起来像没有理由。**更响的理由不等于更真的理由。**）
