...
- dsh 版本：**实时追踪最新版**（所有 dist-tag 里 semver 最大的那个，不是某个固定 tag），整套 `@deepseek-ai/*` 精确钉同一版本，随 dsh 的破坏性重构一起重构（PLAN D11）；**当前锁定 `0.1.5-alpha.1`**（以 `docs/dsh/facts.md` 首行为准）
...
· 0.3 ✅ 防腐层与版本锁 · 0.4 ✅ 总 bundle 与 `stm_hello`（真实 dsh 集成已验）· 0.6 ✅ spike（结清 1/3/5/7，半结清 6）· 0.7 ✅ 规格导出 · 1.1 ✅ si.ts 移植 · 1.2 ✅ 线协议帧层 · 1.2b ✅ 类型码表 · 1.3 ✅ 协议代码生成 · 1.4 ✅ RoleLink TCP 客户端 · 1.5 ✅ 熔断状态机。
**Phase 0 完成**（只差覆盖率门禁）。**Phase 1 进行中，下一步 = 课时 1.6（`ctx.instrument` Service）**。2026-09-09 已升 dsh 到 `0.1.5-alpha.1`。0.5（设置卡）已挪到 1.7。

5 个工作区包、270 条单测 + 4 条 stmsim 集成测试，golden 与 Nanonis 协议表已入仓。逐段清单、每段的停点与验收在 **`docs/EXECUTION.md`**。
...
| **`spec/golden/`** | 与 Python 侧对账的**分母**：515 条技能契约 + 146 条 SI 金样 + 51 条线协议字节金样，由 `tools/spec-export/` 从旧仓导出，**重跑逐字节相同**。不要手改 |
