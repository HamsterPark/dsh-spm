...
**Phase 0 完成**（只差覆盖率门禁）。**技能已做到 411/515（八成），针尖登记表（D-TIP-1 结清）、仪器档案、Z 稳定与粗动驱动四个子系统已落；下一步 = 特异化流程 `_tip_phases`（六个组合技能）+ vision 的 scan_prep 链**。2026-09-13 已升 dsh 到 `0.1.5-rc.2`（接触面零变化；**追踪已切到稳定通道**，见 `docs/dsh/upgrades.md` 的切换点补记）。0.5（设置卡）挪到 1.7 后再挪到 1.10，与 U0 共用客户端机器。

16 个工作区包（含 root）、**5847 条测试**（含 86 条对真 stmsim 的集成测试，模拟器由 vitest globalSetup 自动起停）+ **510 条变异演练全红**，已移植 **411/515** 技能、**88/165** 模块，golden 与 Nanonis 协议表已入仓。逐段清单、每段的停点与验收在 **`docs/EXECUTION.md`**——**Phase 2–8 的分段计划已于 2026-09-10 一次写全**（约 45 个课时）。
...
| **`spec/golden/`** | 与 Python 侧对账的**分母**：515 条技能契约 + 411 个技能的 1630 条调用轨迹 + 146 条 SI 金样 + 51 条线协议字节金样 + 50 步熔断轨迹 + 状态缓存三节（29 步 trace），另有十二台专用驱动器（数值 29 节 / 分析判定 / 晶格 18 节 / paper 数据 95 格 / 扫描解析 77 格 / 撞针 21 脚本 / 环境判定 124 格 / z 轨迹 46 格 / Nanonis 文件 26 格 …），由 `tools/spec-export/` 从旧仓导出，**重跑逐字节相同**。不要手改 |
