...
· 0.3 ✅ 防腐层与版本锁 · 0.4 ✅ 总 bundle 与 `stm_hello`（真实 dsh 集成已验）· 0.6 ✅ spike（结清 1/3/5/7，半结清 6）· 0.7 ✅ 规格导出 · 1.1 ✅ si.ts 移植（146 条金样驱动）。
**Phase 0 完成**（只差覆盖率门禁）。**Phase 1 进行中，下一步 = 课时 1.2（`nanonis-wire` codec）**。0.5（设置卡）已挪到 1.7。

4 个工作区包、164 条测试、golden 已入仓。逐段清单、每段的停点与验收在 **`docs/EXECUTION.md`**。
...
| **`spec/golden/`** | 与 Python 侧对账的**分母**：515 条技能契约 + 146 条 SI 金样，由 `tools/spec-export/` 从旧仓导出，**重跑逐字节相同**。不要手改 |
