...
**Phase 0 完成**（只差覆盖率门禁）。**Phase 2/3 的技能批已做到 329/515，Phase 4 的数值底座与文件读（课时 4.1 / 4.2）已完成；下一步 = 课时 4.3 `vision-classic`**。2026-09-13 已升 dsh 到 `0.1.5-rc.2`（接触面零变化；**追踪已切到稳定通道**，见 `docs/dsh/upgrades.md` 的切换点补记）。0.5（设置卡）挪到 1.7 后再挪到 1.10，与 U0 共用客户端机器。

15 个工作区包（含 root）、**4074 条测试**（含 40 条对真 stmsim 的集成测试，模拟器由 vitest globalSetup 自动起停）+ **216 条变异演练全红**，已移植 **329/515** 技能、**50/165** 模块，golden 与 Nanonis 协议表已入仓。逐段清单、每段的停点与验收在 **`docs/EXECUTION.md`**——**Phase 2–8 的分段计划已于 2026-09-10 一次写全**（约 45 个课时）。
...
| **`spec/golden/`** | 与 Python 侧对账的**分母**：515 条技能契约 + 329 个技能的 1392 条调用轨迹 + 146 条 SI 金样 + 51 条线协议字节金样 + 50 步熔断轨迹 + 状态缓存三节（29 步 trace），另有几台专用驱动器（数值 19 节 / 环境判定 / z 轨迹 / Nanonis 文件 26 格 / 锁相参数组 …），由 `tools/spec-export/` 从旧仓导出，**重跑逐字节相同**。不要手改 |
