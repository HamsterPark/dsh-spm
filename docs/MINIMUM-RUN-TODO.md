# 最小运行调查（2026-09-20，历史记录）

本文保留迁移早期对装载、分发和模型可见性的调查。**当前操作入口已迁至 [受管 STM-Bench 指南](MINIMUM-USABLE.md) 和 [Nanonis 模拟器指南](NANONIS-SIMULATOR.md)**；两者均有 2026-09-21 的安装态与真实模型验收记录，对应不同包哈希和有限工具范围。

后续实施见 [第一版计划](MINIMUM-USABLE-PLAN.md)、[受管入口交接](handoff/minimum-usable-20260921.md) 和 [原生入口交接](handoff/native-nanonis-20260921.md)。下面的步骤、时间估计和未勾选状态保留当时原意，不再作为当前操作清单；完整客户端问题也不能由最小入口的通过结果自动关闭。

> 以下记录写于 2026-09-20；发布准备另见 `RELEASE-TODO.md`。
>
> 当时的事实：开发态已经记录过插件装载、`apply` 执行、依赖注入与工具进入宿主注册表；
