...
（三条边界判据全造在轴上，`hypot(a, 0)` 两边都精确）。推导写在 D-MAP-? 那一条里。
...
同样处理的还有两处（写在 D-MAP-? 里）：`spacing()` 里 `v < 1` 那一支
