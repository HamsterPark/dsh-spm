...
> ⇒ **订正这条订正本身（2026-09-20 晚，写讲义时核出来的）**：这句话说「本文件开篇立的
> 纪律」，而 **`batch-6a.md` 开篇没有这句话** —— 全文 `grep 纪律` 只有这一处，也就是
> 这句订正自己。那条纪律的真出处是 `docs/handoff/survey-remaining.md:34`
> （「判据是**函数级**的：看 `execute` / `validate_params` 实际调到哪些旧仓函数……
> 这条纪律是上一轮两次相反的错换来的」）。
>
> 于是这条订正本身**也是一次没核的引用** —— 它在指责一次没核的引用。
> 而真相比原话更难堪：不是「规矩没约束住同一份文档的后面」，是
> **规矩写在 A 文档，而 B 文档从来不知道有这回事**。
> `batch-6a.md` §1 那张表确实按函数数（「15 函数 / 511 行」），紧接着的 §1.1 就按文件列，
> **同一节两种粒度并排，没有一句话说哪种算数**。
>
> ⇒ **一条只活在默契里的纪律，和没有这条纪律的区别，只在有人没照做的那一天才看得出来。**
>
> 原话留档：「本文件开篇立的纪律是『依赖按**函数级**追，不按 import 行、不按文件名』，
