...
`phaseCrossCorrelation`（`fft.ts:492`）**不能顶**，两条（登记为 D-PHASESHIFT-?）：
...
| 编号 | 一句话 |
|---|---|
| **D-SCANPATH-?** | `ScanAt` 从不返回路径 ⇒ 两段死代码，新旧两仓都是。照移 + 登记 + 一条量着它的测试 |
| **D-COARSE-?** | `phase_shift` 回 `None` 时旧仓 `None / n` 抛 `TypeError` 穿出 `execute`（DoD ⑤：**不照抄**）。本仓给一句拒绝；「扫出来了但读不动」与「扫描失败」分成两句 |
| **D-POINT-?** | `no numeric value in Nanonis reply:` 后面印 body 不印信封（D-SKILL-2 的又一处）；`InstrumentError` 这个类不搬 |
| **D-POINT-2?** | `mean_std` 走 CPython 的 `sum()`（Neumaier），`phase_shift` 走 numpy 的成对 —— 两族不能混 |
| **D-GRID-?** | 超时判据在通用轨迹金样里是死代码；输入在 `batch7b2.json`，而两侧的符号由**睡掉的秒数**保证 |
| **D-OPTICS-?** | `AcquireSignalPoint` 不是 D 档；但 `builtins.optics_scan` 永远到不了 complete |
| **D-PHASESHIFT-?** | 归一化分母 `1e-30`（旧仓）而不是 `100·eps`（skimage）；合成帧那层宽带噪声是 `snr` 能不能比的前提 |
