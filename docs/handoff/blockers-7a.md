...
⚠️ **而这一句兜不住上表第 4 行。** `BiasWiggle` 缺的不是上游的某个函数 ——
再怎么往下追旧仓，也追不到「本仓要先长出一条中止清理通道」这件事。
追得再深，追的仍然只是**上游那一棵树**。
...
| 栏 | 问的是 | 怎么追到底 |
|---|---|---|
| **上游有什么** | 旧仓那棵树上还有哪些函数没落 | 追 `import` / `context.run` 到全部已落或已在清单里 |
| **本仓要先长出什么** | 这个技能要求的能力，本仓这一侧存在吗 | 逐条读它的**纪律句**（注释里那些「必须」「也不例外」），对照本仓有没有那个口 |

第二栏没有机械办法 —— 它要读的是旧仓写给人看的那几句话。
`bias_wiggle.py:116` 的「**abort 也不例外**」就是那样一句：它不是 import，不是调用，
是一条**对宿主的要求**。
