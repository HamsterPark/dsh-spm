...
Continue publication preparation from the separately prepared `dsh-spm-public-ready` checkout. The publication target will be a new, empty, independent `HamsterPark/dsh-spm` repository. At publication time, first confirm that the proposed archive name `HamsterPark/dsh-spm-private-archive` is available; if it is occupied, select another explicit private archive name. Rename the existing repository to that archive name and keep it private.

Before reusing the original name, repoint or disable pushes from the retained original local repository and all nine worktrees sharing its Git configuration so that they can target only the private archive; only the publication checkout should connect to the new repository. Then create the new `HamsterPark/dsh-spm` as private, not as a fork, and do not use the archive as a GitHub import source. Record the immutable GitHub repository ID before and after this sequence: URL and repository name are insufficient because the original URL will be reused. GitHub explains that reusing the old name stops redirects in [Renaming a repository](https://docs.github.com/en/repositories/creating-and-managing-repositories/renaming-a-repository).

Push the reviewed local `master`, and only that branch, while retaining the cleaned 207-commit development history; do not replace it with an orphan snapshot. Do not merge or pull the retained original private history into the publication checkout. The local backup, detailed findings, replacement rules and old/new commit map are not publication files.
...
The remote audit also confirmed that all 207 original commit IDs returned HTTP 200 through GitHub's authenticated API, and the old root instruction file remained readable with a confirmed personal absolute path. Normal clones contain only cleaned history, but remote branch replacement does not remove these server-side objects or cached views. This remains a fact about the private archive, but removing its retained objects is no longer a prerequisite for publishing through the new independent repository. Keep the existing repository private through its rename and afterward. GitHub documents the support-assisted removal process and its eligibility limits in [Removing sensitive data from a repository](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/removing-sensitive-data-from-a-repository). Detailed evidence and the unsent support-request draft remain available as an optional private-archive cleanup path.
...
The existing repository has not been renamed, and the new repository has not been created, pushed or made public. After the new `HamsterPark/dsh-spm` is created privately, verify that its immutable repository ID differs from the renamed archive and that GitHub reports `fork: false`; verify a fresh clone against the reviewed local tree and cleaned history; and check that the 207 original commit IDs and the old root file cannot be read through routes under the new repository. HTTP 403 and 429 do not prove disappearance and must be treated as inconclusive. Only after these checks pass may the new repository be made public under a separate publication instruction. Keep the renamed archive private, and do not publish every local ref or tag by default.
