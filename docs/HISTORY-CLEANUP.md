...
At the earlier 2026-09-21 readiness check, the existing repository had not been renamed and the new independent repository had not been created. The subsequent preparation completed that sequence: the archive retains repository ID `1372557135` and remains private; the new independent `HamsterPark/dsh-spm` has ID `1379303402`, reports `fork: false` and was initially kept private for verification. Original-history local remotes were repointed before reusing the name.
...
The final publication commit was `980e81acf29a2bf376ff864c695fe160f93c5195`, with 214 commits and 668 files. Its four CI jobs, fresh-download comparison and authenticated isolation checks passed. On 2026-09-21, the owner authorized public visibility for the new repository. Its ID remained `1379303402`; anonymous repository/current-commit access succeeded, and the anonymously fetched README matched the published blob. All 207 original commit pages and the known old file returned HTTP 404 without credentials. The archive remained private and returned 404 anonymously.

The [publication record](handoff/publication-ready-20260921.md) separates the initial candidate from the final lifecycle-test correction and publication outcome. The [release checklist](RELEASE-TODO.md) records all publication actions as complete; subsequent documentation-only updates do not change the tested code.
