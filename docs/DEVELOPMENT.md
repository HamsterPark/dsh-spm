...
4. **模拟器集成**：对仪器交互验证实际 stmsim 链路。若某项不适用，说明理由；缺少外部依赖时记录未验证，不能用替身测试冒充集成验证。
...
集成测试由 [vitest.stmsim-setup.ts](../vitest.stmsim-setup.ts) 管理模拟器，使用本地 16501–16504 端口。不要与使用相同端口的实例同时运行。缺少环境变量会明确失败，不会静默跳过。
