...
**Phase 1：1.1 ✅**（`si.ts` + 146 条金样）· **1.2 ✅**（线协议帧层 + 字节金样）。
**下一段 = 课时 1.2b（类型码表）**——原 1.2 因超出 200 行约定拆成两段，理由见下。

仓库现状：5 个工作区包（root / compat / kernel / **nanonis-wire** / bundle），
**186 条测试**，`pnpm install --frozen-lockfile` / `pnpm build` / `pnpm test` 全绿。锁定 dsh `0.1.2-rc.1`。
golden 已入仓（515 技能 + 146 SI 用例 + 15 条线协议字节金样，重跑逐字节相同）。
