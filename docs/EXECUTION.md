...
git 的三方合并不是按行比，是按 **hunk** 比：两处改动之间**至少要隔开几行没变的上下文**，
否则它们会被归进同一个 hunk，而同一个 hunk 上的两份改动就是一处冲突。

第一轮那三个常量自动合上，不是因为「一条一行」，是因为每条支线只填三个里的一个、
另两行离得够远（中间还夹着各自的注释）。**那次是运气，我当成了方法。**
⇒ 锚点买的不是「不同的行」，是**「中间有几行谁都不碰的东西」**。
